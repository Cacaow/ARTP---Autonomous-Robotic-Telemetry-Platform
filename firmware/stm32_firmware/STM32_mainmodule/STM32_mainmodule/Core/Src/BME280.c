...
void Reset_BME280(void){
	...
	uint8_t data = 0xB6;
	HAL_I2C_Mem_Write(&BME280_I2C_PORT, BME280_ADDR, RESET_REG_ADDR, 1, &data, 1, HAL_MAX_DELAY);
	HAL_Delay(500);

	//Checking for is reset process done
	uint8_t id;
	HAL_I2C_Mem_Read(&BME280_I2C_PORT, BME280_ADDR, CHIP_ID_REG_ADDR, 1, &id, 1, HAL_MAX_DELAY);
...
HAL_StatusTypeDef BME280_SleepMode(void){
	...
	return HAL_I2C_Mem_Write(&BME280_I2C_PORT, BME280_ADDR, CTRL_MEAS_REG_ADDR, 1, &init, 1, 1000);
...
void Calibdata_BME280(void){
	...
	HAL_I2C_Mem_Read(&BME280_I2C_PORT, BME280_ADDR, CALIB_DATA00_25_BASEADDR, 1, CalibrationData1, 26, HAL_MAX_DELAY); //From 0x88 to 0xA1
	HAL_I2C_Mem_Read(&BME280_I2C_PORT, BME280_ADDR, CALIB_DATA26_41_BASEADDR, 1, CalibrationData2, 7, HAL_MAX_DELAY); //From 0xE1 to 0xE7
...
Raw_Data_t RawdataBME280(void){
	...
	HAL_I2C_Mem_Read(&BME280_I2C_PORT, BME280_ADDR, RAWDATA_BASEADDR, 1, rawData, 8, 1000);
...
/**
  ...
  * @retval None
  */
void InitBME280(BME280_Init_t BME280Init){
	...
	HAL_I2C_Mem_Write(&BME280_I2C_PORT, BME280_ADDR, CTRL_HUM_REG_ADDR, 1, &init, 1, 1000);
	HAL_Delay (100);
	init=0;
	...
	HAL_I2C_Mem_Write(&BME280_I2C_PORT, BME280_ADDR, CTRL_MEAS_REG_ADDR, 1, &init, 1, 1000);
	HAL_Delay (100);
	init=0;
